...
(defmethod + Collection [& args] (reduce concat (remove nil? args)))
(defmethod + Map [& args] (reduce deep-merge (remove nil? args)))
(defmethod + nil [_] nil)

(defmulti  - (fn [& xs] (type (first (remove nil? xs)))))
(defmethod - Number [& args] (reduce clojure.core/- (remove nil? args)))
(defmethod - String [& args] (reduce (fn [s m] (str/replace-first s (str m) "")) (remove nil? args)))
